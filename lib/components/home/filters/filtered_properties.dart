...
class FilteredProperties extends StatefulWidget {
  ...
  FilteredProperties({required this.category_id, required this.subcategory_id, required this.name});
  @override
...
class _FilteredPropertiesState extends State<FilteredProperties> {
  ...
  Future<List<PropertyModel>> getAllFacilities() async {
    Map data = {
      ...
      "sub_category_id": widget.subcategory_id.toString(),
    };
          ...
          child: Row(
            children: [
              ...
              Padding(
                ...
                child: Text("Getting " + widget.name),
              )
            ],
          ),
        ),
          ...
          builder: (context, snapshot) {
            if(listCount == 0){
              return _isLoading ? loader2 : Center( child: Container(
                child: const Text("There are no facilities to show", style: TextStyle(fontSize: 16.0),),
              ));
            }else{
